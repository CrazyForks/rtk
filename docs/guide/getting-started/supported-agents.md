...
When `PI_CODING_AGENT_DIR` is set, the global Pi and OMP targets can alias to one file. RTK records whether Pi, OMP, or both agents were installed for that alias in an adjacent `rtk.ts.rtk-agents` state file. OMP-targeted commands and detected Pi/OMP shared files disclose the alias; configuration-directory checks (`.omp` in the current project or `~/.omp`, and their Pi equivalents) remain a fallback for older installs. Uninstalling a detected shared file asks before removing it, so confirm that neither agent should use it first (or pass `--auto-patch` to approve). A declined shared uninstall, including `--no-patch`, exits nonzero; `--dry-run` remains a successful preview. RTK currently targets OMP's default profile and `.omp` project directory; named OMP profiles and custom `PI_CONFIG_DIR` locations are not auto-detected.
